...
#define M_PER_WARP 4  // Each warp computes 4 M outputs

__global__ void gemv_warp_multi_m(
    ...
    // Each warp handles M_PER_WARP consecutive M values
    int m_base = (blockIdx.x * WARPS_PER_BLOCK + warp_id) * M_PER_WARP;
    int l = blockIdx.z;

    // Cooperative load B and sfb into shared memory
    ...
    // Accumulators for M_PER_WARP outputs
    ...
    for (int pair = lane_id; pair < K_pairs; pair += WARP_SIZE) {
        int block_k = pair / PAIRS_PER_SCALE;

        // Load B and sfb from shared (same for all M values)
        __nv_fp4x2_storage_t b_pair = B_sh[pair];
        __half_raw sb_raw = __nv_cvt_fp8_to_halfraw(sfb_sh[block_k], __NV_E4M3);
        ...
        float b0 = __half2float(__hmul(__low2half(h2b), sb));
        float b1 = __half2float(__hmul(__high2half(h2b), sb));

        // Process each M value this warp owns
        #pragma unroll
        for (int mi = 0; mi < M_PER_WARP; mi++) {
            ...
            float a0 = __half2float(__hmul(__low2half(h2a), sa));
            float a1 = __half2float(__hmul(__high2half(h2a), sa));

            acc[mi] += a0 * b0 + a1 * b1;
        }
    }

    // Warp reduction for each M output
    #pragma unroll
    for (int mi = 0; mi < M_PER_WARP; mi++) {
        #pragma unroll
        for (int offset = WARP_SIZE / 2; offset > 0; offset /= 2) {
            acc[mi] += __shfl_down_sync(0xffffffff, acc[mi], offset);
        }
    }

    // Lane 0 writes all M results
    if (lane_id == 0) {
        #pragma unroll
        for (int mi = 0; mi < M_PER_WARP; mi++) {
            int m = m_base + mi;
            if (m < M) {
                C[M * l + m] = __float2half(acc[mi]);
            }
        }
    }
}
...
void nvfp4_gemv(
    ...
    int M_per_block = WARPS_PER_BLOCK * M_PER_WARP;
    dim3 grid(CEIL_DIV(M, M_per_block), 1, L);
    
    size_t smem_size = K_pairs + K_blocks;

    gemv_warp_multi_m<<<grid, threadsPerBlock, smem_size>>>(
